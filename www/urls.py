...
def index():
    blogs = Blog.find_all()
    # user = User.find_first('where email=?', 'person@blog.com')
    user = ctx.request.user
    if user != None:
        logging.info('user: '+user.name)
    else:
        logging.info('user: None')
...
def register_user():
    ...
    user.insert()
    # make session cookie:
    cookie = make_signed_cookie(user.id, password, None)
