...
env.user = 'lambda'
env.sudo_user = 'root'
env.hosts = ['192.168.204.201']

db_user = 'www-data'
db_password = 'www-data'

_TAR_FILE = 'dist-web4py.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/web4py'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-web4py-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick web4py > %s' % (db_user, db_password, f))
...
def deploy():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        # sudo('chown -R www-data:www-data %s' % _REMOTE_BASE_DIR)
        sudo('chown -R root:root %s' % _REMOTE_BASE_DIR)
    with settings(warn_only=True):
        sudo('supervisorctl stop web4py')
        sudo('supervisorctl start web4py')
...
def restore2local():
    ...
    sqls = [
        'drop database if exists web4py;',
        'create database web4py;',
        'grant select, insert, update, delete on web4py.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s web4py < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
